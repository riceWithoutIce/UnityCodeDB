...
Assert-Equal -Actual $packageManifest.version -Expected "0.2.5-preview.1" -Label "Package version"
...
Assert-Equal -Actual $payloadManifest.payload_version -Expected "poc.28" -Label "Payload version"
Assert-Equal -Actual $payloadManifest.payload_sequence -Expected 28 -Label "Payload sequence"
Assert-Equal -Actual $payloadManifest.generation_id -Expected "poc.28" -Label "Payload generation"
...
$generationSourcePrefix = "Generations/poc.28/"
$generationTargetRoot = "AIWork/.runtime/codedb/host/generations/poc.28"
...
$expectedRetiredTargets = @(foreach ($retiredGenerationId in @("poc.22", "poc.23", "poc.24", "poc.25", "poc.26", "poc.27")) {
...
Assert-Equal -Actual $actualRetiredTargets.Count -Expected 126 -Label "Retired target count"
Assert-Equal `
...
$generationSourceRoot = Join-Path $payloadRoot "Generations\poc.28"
