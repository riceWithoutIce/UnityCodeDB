...
$script:CodedbRequiredProviderVersion = "0.5.0-28e3912"
...
function Read-CodedbProviderManifest {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        Throw-CodedbPrerequisiteError -ReasonCode "PROVIDER_MISSING" -Message "CodeDB Provider 0.5.0-28e3912 is missing. Install the reviewed machine Provider, then let Unity recheck automatically."
    }
...
function Assert-CodedbMachineProvider {
    ...
    if (-not (Test-Path -LiteralPath $providerRoot -PathType Container) -or
        ...
        Throw-CodedbPrerequisiteError -ReasonCode "PROVIDER_MISSING" -Message "CodeDB Provider 0.5.0-28e3912 is missing. Install it under %LOCALAPPDATA%\Rice\CodeDB\providers\0.5.0-28e3912, then let Unity recheck automatically."
    }
    ...
    try {
        ...
        if ((Get-CodedbProviderManifestInt32 -Properties $manifest -Name "schema_version") -ne 1 -or
            ...
            Throw-CodedbPrerequisiteError -ReasonCode "PROVIDER_INCOMPATIBLE" -Message "The installed CodeDB Provider identity or protocol is incompatible with this Package. Install the reviewed Provider 0.5.0-28e3912 build, then let Unity recheck automatically."
        }
        ...
        if ($expectedSha256 -cnotmatch '^[0-9a-f]{64}$' -or
            ...
            Throw-CodedbPrerequisiteError -ReasonCode "PROVIDER_HASH_MISMATCH" -Message "The installed CodeDB Provider hash does not match its manifest. Reinstall the reviewed Provider 0.5.0-28e3912 build, then let Unity recheck automatically."
        }
    } catch {
