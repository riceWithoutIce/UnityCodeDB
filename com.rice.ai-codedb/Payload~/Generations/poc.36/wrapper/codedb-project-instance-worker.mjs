...
const REQUIRED_PROVIDER = Object.freeze({
  ...
  sha256: "38c7d07dde2fa9e322ac0dcbb5ca8961921c8ea6aad548e6bd36e2277752e5e7",
  protocol: "codedb-cli-v1",
  capabilityContract: "codedb-search-tools-v1",
...
function resolveMachineProvider() {
  ...
  if (!fs.existsSync(providerRoot) || !fs.statSync(providerRoot).isDirectory()
    ...
    throw new Error("[MISSING_PREREQUISITE] CodeDB Provider 0.5.0-28e3912-c2 is missing under %LOCALAPPDATA%/Rice/CodeDB/providers/0.5.0-28e3912-c2.");
  }
  ...
  if (Object.keys(manifest).length !== expectedNames.length
    ...
    throw new Error("[MISSING_PREREQUISITE] CodeDB Provider manifest properties do not match schema 2.");
  }
  const identityCurrent = manifest.schema_version === REQUIRED_PROVIDER.schemaVersion
    ...
    && manifest.sha256 === REQUIRED_PROVIDER.sha256
    && manifest.protocol === REQUIRED_PROVIDER.protocol
    && manifest.capability_contract === REQUIRED_PROVIDER.capabilityContract
    && manifest.source === REQUIRED_PROVIDER.source;
  if (!identityCurrent) {
    throw new Error("[MISSING_PREREQUISITE] The installed CodeDB Provider identity, protocol, or capability contract is incompatible.");
  }
