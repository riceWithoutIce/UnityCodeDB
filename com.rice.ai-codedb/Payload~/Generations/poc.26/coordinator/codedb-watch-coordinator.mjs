...
const MANUAL_RUNTIME_FILE = "manual-runtime.json";
const EDITOR_LEASE_DIRECTORY = "editor-leases";
const IPC_TIMEOUT_MS = 1500;
const PROVIDER_REQUEST_TIMEOUT_MS = 90000;
const PROVIDER_QUERY_DRAIN_TIMEOUT_MS = PROVIDER_REQUEST_TIMEOUT_MS + 5000;
...
function buildRuntimeContext(rawOptions) {
  ...
  return {
    ...
    manualRuntimePath: path.join(lifecycleRoot, MANUAL_RUNTIME_FILE),
...
function buildLaunchContext(rawOptions) {
  ...
  context.generationId = parseGenerationId(rawOptions.generationId ?? GENERATION_ID);
  ...
  context.legacyAdapterBuilder = hasAdapterBuilder
    ? path.join(context.root, "AIWork", "codedb", "scripts", "build-codedb-project-text-adapter.ps1")
    : null;
  context.legacyAdapterWorker = hasAdapterWorker
    ? path.join(context.root, "AIWork", "codedb", "scripts", "run-codedb-project-text-adapter-worker.ps1")
    : null;
  ...
  if (context.adapterEnabled) {
    ...
    assertPathInside(context.legacyAdapterBuilder, context.root, "legacy adapter builder alias");
    if (context.adapterWorker) {
      ...
      assertPathInside(context.legacyAdapterWorker, context.root, "legacy adapter worker alias");
    }
  }
...
function parseGenerationId(value) {
  const parsed = String(value ?? "");
  if (!/^[A-Za-z0-9._-]{1,64}$/.test(parsed) || parsed === "." || parsed === "..") {
    throw new Error(`Invalid generation id: ${value}`);
  }
  return parsed;
}
...
async function readLifecycleSnapshot(context, removeInvalid = true) {
  ...
  const activeSessionIds = new Set(inspection.leases.map((lease) => lease.session_id));
  const manual = readManualRuntime(context, activeSessionIds);
  const policyState = desired?.desired_state ?? "unknown";
  const desiredState = manual?.mode === "started"
    ? "enabled"
    : (manual?.mode === "stopped" ? "disabled" : policyState);
  return {
    desiredState,
    policyState,
    manualMode: manual?.mode ?? "none",
    leases: inspection.leases,
    invalid: inspection.invalid
  };
}

function readManualRuntime(context, activeSessionIds) {
  const document = readJsonFile(context.manualRuntimePath);
  ...
  const valid = document?.schema_version === LIFECYCLE_SCHEMA_VERSION
    ...
    && normalizePath(document?.project_root) === normalizePath(context.root)
  ...
  return sessionIds.some((sessionId) => activeSessionIds.has(sessionId)) ? document : null;
}
...
function assertStatusMatchesContext(status, context, label) {
  ...
  for (const [field, expected] of [
    ["adapter_builder", context.legacyAdapterBuilder],
    ["generation_adapter_builder", context.adapterBuilder],
    ["adapter_manifest", context.adapterManifest],
    ["adapter_worker", context.legacyAdapterWorker],
    ["generation_adapter_worker", context.adapterWorker]
  ]) {
    if (normalizePath(status?.[field]) !== normalizePath(expected)) {
      throw new Error(`${label} metadata mismatch for ${field}; stop it before starting this configuration.`);
    }
...
async function runStatus(rawOptions) {
  ...
  const lifecycle = context.root
    ...
    : { desiredState: "unknown", policyState: "unknown", manualMode: "none", leases: [], invalid: [] };
  ...
  printResult(action === "stale" ? "STALE" : "STOPPED", {
    action,
    generation_id: state?.generation_id ?? rawOptions.generationId ?? null,
    reason_code: reasonCode,
    desired_state: lifecycle.desiredState,
    policy_state: lifecycle.policyState,
    manual_mode: lifecycle.manualMode,
...
async function runDaemon(rawOptions) {
  const context = buildLaunchContext(rawOptions);
  await assertLifecycleDemand(context);
  const hostUseLease = acquireCodedbHostUseLease(context.root, "watcher", context.generationId);
  try {
...
async function runOwnedDaemon(context) {
  ...
  const createProviderQueryFlight = (request, key) => {
    ...
    const execute = async () => {
      ...
      if (!["ready", "draining"].includes(daemonState.provider_state) || !providerRpc) {
  ...
  const cleanup = async (reason, exitCode = 0) => {
    ...
    shutdownPromise = (async () => {
      stopping = true;
      daemonState.provider_state = provider && provider.exitCode === null ? "draining" : "stopping";
      if (context.adapterEnabled) {
        daemonState.adapter_state = "draining";
      }
      if (context.adapterWorker) {
        daemonState.adapter_worker_state = "draining";
      }
      ...
      const serverClosed = server
        ...
        : Promise.resolve();
      const queryDrainTail = providerQueryTail;
      const queriesDrained = await waitForPromiseCompletion(queryDrainTail, PROVIDER_QUERY_DRAIN_TIMEOUT_MS);
      if (!queriesDrained) {
        daemonState.last_error = `Timed out after ${PROVIDER_QUERY_DRAIN_TIMEOUT_MS} ms draining accepted Provider queries.`;
        log(`provider_query_drain_timeout timeout_ms=${PROVIDER_QUERY_DRAIN_TIMEOUT_MS} inflight=${providerQueryFlights.size}`);
      } else {
        log(`provider_query_drain_complete inflight=${providerQueryFlights.size}`);
      }
  ...
  const scanEditorDemand = async () => {
    ...
    try {
      ...
      daemonState.policy_state = lifecycle.policyState;
      daemonState.manual_mode = lifecycle.manualMode;
  ...
  const launchProvider = async () => {
    ...
    const initialize = await providerRpc.request("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "codedb-watch-coordinator", version: "0.2.4-preview.4" }
    }, PROVIDER_REQUEST_TIMEOUT_MS);
...
async function recoverStaleRuntime(context) {
  ...
  const staleProcessContext = validateStaleStateContext(state, context);
  ...
  for (const [processId, kind] of adapterProcesses) {
    ...
    const identityMatches = kind === "worker"
      ? adapterWorkerIdentityMatches(identity, staleProcessContext)
      : adapterBuilderIdentityMatches(identity, staleProcessContext);
  ...
  if (isProcessAlive(state.provider_pid)) {
    ...
    if (!identity || !providerIdentityMatches(identity, staleProcessContext)) {
...
function validateStaleStateContext(state, context) {
  for (const [field, expected] of [
    ["root", context.root],
    ["provider_executable", context.provider],
    ["provider_config", context.config],
  ...
  const staleGenerationId = parseGenerationId(state.generation_id);
  const staleGenerationRoot = path.join(
    context.root,
    "AIWork",
    ".runtime",
    "codedb",
  ...
  const staleAdapterBuilder = path.join(
    staleGenerationRoot,
    "scripts",
    "build-codedb-project-text-adapter.ps1"
  );
  const staleAdapterWorker = context.adapterWorker
    ? path.join(staleGenerationRoot, "scripts", "run-codedb-project-text-adapter-worker.ps1")
    : null;
  for (const [field, expected] of [
    ["adapter_builder", context.legacyAdapterBuilder],
    ["generation_adapter_builder", staleAdapterBuilder],
    ["adapter_manifest", context.adapterManifest],
    ["adapter_worker", context.legacyAdapterWorker],
    ["generation_adapter_worker", staleAdapterWorker]
  ]) {
    if (normalizePath(state[field]) !== normalizePath(expected)) {
      throw new Error(`Stale coordinator metadata mismatch for ${field}; refusing cleanup.`);
    }
  ...
  return {
    ...
    adapterBuilder: staleAdapterBuilder,
    adapterWorker: staleAdapterWorker
  };
}
...
function sanitizeStatus(state) {
  return {
    ...
    generation_adapter_builder: state.generation_adapter_builder,
    generation_adapter_worker: state.generation_adapter_worker,
...
async function waitForPromiseCompletion(promise, timeoutMs) {
  let timer;
  try {
    return await Promise.race([
