...
try {
    ...
    { $_ -in @("Ensure", "Enable", "Start", "Restart") } {
        ...
        try {
            if (($isAutomaticEnsure -and -not $MaterializerHandoff) -or $isPolicyEnable) {
            ...
            } elseif (-not (Test-Path -LiteralPath $watchConfigPath -PathType Leaf)) {
                throw "Materializer handoff cannot create missing watch configuration; complete Setup first."
            }
